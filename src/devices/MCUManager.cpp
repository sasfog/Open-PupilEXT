...
//        }
        ...
        illumTempChecks.push_back(temps);

        if(!hasDeltaTimeData && checkIntervalSec * illumTempChecks.size() >= illumWarmupStableDeltaTime) {
            hasDeltaTimeData = true;
            emit illumWarmupHasDeltaTimeData();
        }

        if(hasDeltaTimeData &&
           temps[0] - illumTempChecks[illumTempChecks.size()-(illumWarmupStableDeltaTime/checkIntervalSec)][0] <= illumWarmupStableDeltaTemp &&
           temps[1] - illumTempChecks[illumTempChecks.size()-(illumWarmupStableDeltaTime/checkIntervalSec)][1] <= illumWarmupStableDeltaTemp ) {

            float cavg0 = MCUManager::MINIMUM_ILLUMINATOR_TEMPERATURE;
            float cavg1 = MCUManager::MINIMUM_ILLUMINATOR_TEMPERATURE;
            for(auto chk : illumTempChecks) {
                cavg0 = (cavg0 + chk[0])/2;
                cavg1 = (cavg1 + chk[1])/2;
            }
...
void MCUManager::setRunning(bool running)
{
