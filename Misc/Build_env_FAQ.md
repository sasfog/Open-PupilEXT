...
Linking halts with an error stating that AGL cannot be found.

SOLUTION:

If you inspect the linker command, it will contain some part referencing AGL for linking: ".../Users/myuser/Qt/6.8.3/macos/lib/QtGui.framework/Versions/A/QtGui  -framework AppKit  -framework AGL  -framework OpenGL  -framework ImageIO  -framework Metal  -framework ApplicationServices  /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX26.2.sdk/usr/lib/libcups.tbd ...". This means that the Qt version that PupilEXT is being built with, was built with a dependency to AGL on Mac. AGL is a deprecated Apple-specific OpenGL-like library, which was only kept for legacy support, and was removed from Qt starting from Qt 6.8.4 LTS. So one part of the problem is Qt version. The other part of the problem is that your MacOS SDK on the host machine lacks linkable files for this AGL, which can easily happen if you are using anything newer than Xcode 16.4 with its default supplied MacOS SDK. One solution is to install Xcode 16.4. You do not need to downgrade your current version, just install 16.4 from www.xcodereleases.com, then change the default Xcode version on the computer by executing `sudo xcode-select -s /Applications/Xcode_16_4.app/Contents/Developer`. Now you just need to invalidate CMake cache and reconfigure the project, then build should pass. Once Qt dependency version will be raised in a later commit, this error should disappear.
