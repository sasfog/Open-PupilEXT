   ...
   methods (Access = public)
      ...
      function setHardwareTriggeringRuntimeLength(obj, runtimeLengthMinutes)
          if (runtimeLengthMinutes < 0)
            error('Could not set hardware triggering runtime length (for the image acquisition triggers generated by the MCU upon request), invalid numeric input supplied');
          end
          if ~obj.Enabled, return, end
      ...
      function setHardwareTriggeringFramerate(obj, fps)
          if (fps < 1)
            error('Could not set hardware triggering framerate (which is generated by the MCU upon request), invalid numeric input supplied');
          end
          if ~obj.Enabled, return, end
          obj.sendRaw(['IHF ' num2str(fps)]);
      end

      function setHardwareTriggeringFramerateLimitingEnabled(obj, state)
          if ~obj.Enabled, return, end
          if state
            obj.sendRaw('IHC true');
          else
            obj.sendRaw('IHC false');
          end
      end

      function setHardwareTriggeringFramerateLimit(obj, fps)
          if (fps < 1)
            error('Could not set hardware triggering framerate limiting, invalid numeric input supplied');
          end
          if ~obj.Enabled, return, end
          obj.sendRaw(['IHV ' num2str(fps)]);
      end
